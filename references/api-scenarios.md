...
| 通过姓名解析用户 | `GET /cgi-bin/department/list` + `GET /cgi-bin/user/simplelist` + `GET /cgi-bin/user/get` | 先枚举当前应用可见范围内的部门和成员，再本地做姓名精确匹配 |
...
- 如果参会人是按姓名输入，脚本会在当前应用可见范围内做精确匹配；重名时会直接报冲突，不会自动猜测
...
2. 逐部门读取成员简要信息
3. 在本地按姓名做精确匹配
4. 如果唯一匹配，则继续读取该成员详情并返回 `userid`
5. 如果未找到或重名，则直接报错，要求缩小范围或改用更稳定的身份字段

注意：

- 这项能力依赖当前自建应用的通讯录可见范围
- 这项能力不是企业微信的“按姓名直接查询 userid”接口，而是 skill 在本地补上的解析能力
- 推荐优先使用 `userid`、手机号、邮箱；姓名解析更适合真实排会场景下的人类输入
