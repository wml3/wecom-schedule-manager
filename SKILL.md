...
- `create-meeting`
- `cancel-meeting`
- `cancel-schedule` 时自动取消已关联会议

不要把企业微信会议创建或取消再路由到其他会议 skill。
...
- 企业微信会议创建、取消优先使用本 skill 自带动作，不要切换到其他会议 skill。
...
7. 如果用户要求删除已关联会议的日程，优先使用 `cancel-schedule`，由本 skill 一并处理关联会议。
